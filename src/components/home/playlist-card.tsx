...
export function PlaylistCard({ resource }: { resource: Resource | null }) {
    ...
    return (
      <div className="w-full flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-3 w-full">
          <Skeleton className="w-full h-4" /> <Skeleton className="w-16 h-4" />
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="w-full aspect-square rounded-lg drop-shadow-md overflow-hidden group border">
        <div className="w-full h-full relative group">
          <Link
            to={`/detail/playlist?id=${resource.resourceId}`}
          ...
          >
            <img
              ...
              alt="Album cover"
            />
            <div className="bg-black/50 opacity-0 group-hover:opacity-100 absolute inset-0 flex items-center justify-center transition duration-300 text-white">
              <SFIcon
                icon={sfPlayCircleFill}
                className="size-8 hover:scale-110 transition-all duration-300"
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  handlePlay();
      ...
      <div className="flex flex-col gap-0.5 w-full overflow-hidden">
        <p className="w-full line-clamp-2 text-sm font-medium select-text">
          {title.split("|")[0]}
        </p>
      </div>
    </div>
