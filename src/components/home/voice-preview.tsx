  ...
  return (
    <div className="flex gap-4 justify-between group">
      <div className="w-16 h-16 rounded-sm overflow-hidden relative shrink-0 border">
        ...
        <div className="opacity-0 group-hover:opacity-100 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white transform transition-all duration-300 ease-in-out">
          <SFIcon
            icon={sfPlayCircleFill}
            className="size-6 hover:scale-110 transition-transform text-white"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 opacity-0 group-hover:opacity-100 shrink-0 pr-6">
        <ArrowDownload24Regular className="size-5 text-black/60 hover:text-black/80" />
        <MoreHorizontal24Regular className="size-5 text-black/60 hover:text-black/80" />
      </div>
    </div>
  );
}
